...
public class DropdownParserTask extends UpdateTask {
    ...
    DropdownParserTask(@NonNull Context context) {
        super(context);
    ...
    private void parsePage(Document doc) {
        ...
        try {
            ...
            for (Element option : forums) {
                ...
                // if this new forum isn't as deep as the previous one on the stack, we need to back up.
                // pop them off until we find a forum on the same or higher level
                ...
                if (forumDepth > previousForum.depth) {
                    // the new forum is below the previous one, so take that as the parent
                    // and we'll add this forum below it when we push it to the stack
                } else if (forumDepth == previousForum.depth) {
                    // the new forum is a sibling of the one on the stack - remove that so
                    // this will take its place as the last added on this level
                    parsedForumStack.pop();
                    previousForum = parsedForumStack.peek();
                }
